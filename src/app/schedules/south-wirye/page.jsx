...
export default function SouthWiryeSchedulePage() {
  ...
  const branchName = '남위례';
  const branch = 'namwirye';
  ...
  useEffect(() => {
    const fetchScheduleData = async () => {
      ...
      console.log('🔍 [SouthWiryeSchedulePage] 시간표 데이터 요청 시작:');
      ...
      try {
        ...
        console.log('✅ [SouthWiryeSchedulePage] 시간표 데이터 로드 성공:');
        ...
        console.log(`📊 [SouthWiryeSchedulePage] ${branchName} 시간표 렌더링 준비 완료:`, {
          '초등부': gradeData['초등부'].length,
          '중등부': gradeData['중등부'].length,
          '고등부': gradeData['고등부'].length
        });
      } catch (err) {
        console.error('❌ [SouthWiryeSchedulePage] 시간표 불러오기 오류:', err);
