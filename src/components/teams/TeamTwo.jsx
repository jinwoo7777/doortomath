...
export const TeamTwo = ({ content }) => {
  ...
  // props에서 받은 content와 기본값 병합
  const teamData = {
    ...defaultContent,
    ...content,
    teamMembers: content?.teamMembers || defaultContent.teamMembers
  };
              ...
              <div className="td_section_heading td_style_1">
                ...
                <p className="td_section_subtitle td_fs_18 td_mb_43">
                  {teamData.description.split('\n').map((line, index) => (
                    <React.Fragment key={index}>
                      {line}
                      {index < teamData.description.split('\n').length - 1 && <br />}
                    </React.Fragment>
                  ))}
                </p>
                <Link
                  href={teamData.buttonLink}
                  className="td_btn td_style_1 td_radius_30 td_medium"
                >
                  <span className="td_btn_in td_white_color td_accent_bg">
                    <span>{teamData.buttonText}</span>
                    <svg
                      width="19"
                      height="20"
                      viewBox="0 0 19 20"
          ...
          >
            <div className="row td_gap_y_24 td_row_gap_30">
              {teamData.teamMembers.map((member, index) => (
                <div className="col-md-6" key={member.id || index}>
